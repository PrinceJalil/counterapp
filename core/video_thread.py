...
class VideoThread(QThread):
    ...
    count_updated_signal = pyqtSignal(int, dict)
    ...
    def _handle_detection(self, frame: np.ndarray):
        ...
        classwise_counts = {}
        if hasattr(self.counter, "classwise_count"):
            for cls, counts in self.counter.classwise_count.items():
                classwise_counts[cls] = counts.get('IN', 0)
                
        self.count_updated_signal.emit(total, classwise_counts)
